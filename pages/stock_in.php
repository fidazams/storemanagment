...
// Fetch suppliers for the party selection
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    foreach ($product_ids as $i => $product_id) {
        if ($quantities[$i] > 0) {
            ...
            $stmt = $conn->prepare("INSERT INTO stock_transactions (type, product_id, quantity, unit_price, total_price, party_id, invoice_no, date, paid_amount, due_amount, chalan_cost) VALUES ('in', ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
    ...
    if ($chalan_cost > 0) {
        $stmt = $conn->prepare("INSERT INTO costs (field, amount, details, reference, date) VALUES ('Chalan', ?, 'Chalan cost from stock in', ?, ?)");
                    ...
                    <table class="table table-bordered" id="productTable">
                        ...
                        <tbody>
                            <?php while ($product = $products->fetch_assoc()) { ?>
                                <tr>
                                    <td><input type="checkbox" class="product-checkbox" value="<?php echo $product['id']; ?>" data-name="<?php echo htmlspecialchars($product['name']); ?>" data-rate="<?php echo $product['buy_rate']; ?>" onchange="updateSelectedProducts()"></td>
